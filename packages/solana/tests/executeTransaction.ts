...
describe('Transaction Execution', async () => {
  ...
  it('Should should execute transaction', async () => {
    await program.rpc.executeTransaction({
      ...
      remainingAccounts: program.instruction.setOwners
        ...
        // @ts-ignore
        .map((meta) =>
          meta.pubkey.equals(walletSigner) ? { ...meta, isSigner: false } : meta
        )
        .concat({
    ...
    assert.equal(tx.didExecute, true);
    let walletAcc = await program.account.wallet.fetch(wallet.publicKey);
    assert.deepEqual(walletAcc.owners, [
      ownerA.publicKey,
      ownerB.publicKey,
      ownerC.publicKey,
    ]);
